...
(function($) {
    $.fn.selectFilter = function(options) {
        var defaults = {
            ...
            // text to display as placeholder when no data
            'noDataPlaceholder': 'No hay datos',
        ...
        var filterElement = $('#input_' + name);
        filterElement.css({'display': 'block'});
        filterElement.css('width', widthStr);
        ...
        $('#input_' + name).on('keyup', function() {
            ...
            keyDelayTimeout = setTimeout(function() {
                ...
                if (self.children().length === 0) {
                    self.append('<option value="-1">' + options.noDataPlaceholder + '</option>');
                }
